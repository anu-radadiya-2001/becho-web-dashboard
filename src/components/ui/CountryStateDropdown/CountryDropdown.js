...
const CountryDropdown = (props) => {
  ...
  const [countries, setCountries] = useState(filterCountries(CountryRegionData, props.priorityOptions, props.whitelist, props.blacklist));
  ...
  function getCountries() {
    ...
    return countries.map((country) => {
		return(
      <option className="dark:text-blacklight" value={valueType === 'short' ? country.countrySlug : country.countryName} key={country.countrySlug}>
  ...
  function getDefaultOption() {
    return (
      <option value="India" key="default">
        India
      </option>
    );
  }
  ...
  return (
    <div className="relative">
      <select className={`${props.className} w-full cursor-pointer appearance-none dark:bg-transparent focus:outline-none"`} onChange={(e) => props.onChange(e.target.value)}>
        {getDefaultOption()}
        {getCountries()}
      </select>
